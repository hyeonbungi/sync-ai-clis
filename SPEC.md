...
- `--json`: `[{id, display, installed, before, after, action, result, reason}]`
...
- **채널 자동화 현황**: GitHub Releases·brew formula·npm은 태그에서 전자동, winget은 첫 버전 머지 후 자동(WinGet Releaser), **scoop 매니페스트는 수동 갱신**(자동화는 backlog).
...
- [ ] **버전 비교**: "이미 최신"을 어떻게 표기할지(업데이트 명령은 대개 멱등이므로 전·후 버전 동일이면 "최신"으로 표시)

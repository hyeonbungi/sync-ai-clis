...
| Codex (OpenAI) | `codex` | ✅ | ✅ | ✅ | `chatgpt.com/codex/install.sh`/`.ps1` | `codex update`(mac/linux) · Win native는 guarded `install.ps1` 재실행 | npm `@openai/codex` · brew cask `codex`(mac만) |
...
- **Windows PowerShell 호출**: `irm…|iex` 류는 `powershell -NoProfile -ExecutionPolicy Bypass -Command "…"`로 감싸 실행(Windows PowerShell 5 vs `pwsh` 차이 고려). 원격 `install.ps1` 실행 세션에는 `Get-FileHash`가 없을 때만 .NET SHA256 기반 fallback 함수를 먼저 정의한다. fallback은 `-Path`와 `-LiteralPath` 호출을 모두 받아야 한다(Codex 공식 설치 스크립트는 `Get-FileHash -LiteralPath ...`로 archive를 검증한다). Claude Code·Codex 등 공식 설치 스크립트가 해시 검증에 `Get-FileHash`를 사용하므로, 사용자 PowerShell 환경에서 `Microsoft.PowerShell.Utility` 자동 로드가 깨져도 설치가 진행되게 하기 위함이다. 설치 URL은 여전히 레지스트리의 공식 HTTPS 상수만 사용한다.
...
- **update** 설치출처별: brew cask→`brew upgrade --cask codex` · npm→`npm i -g @openai/codex@latest` · native(mac/linux)→`codex update` (공식 self-update 서브커맨드, TD-003) · native(Windows)→`$env:CODEX_NON_INTERACTIVE=1; irm https://chatgpt.com/codex/install.ps1 | iex` 를 `Command::powershell_installer`로 실행(2026-07-02, TD-003 보완). Codex Windows updater가 내부적으로 `powershell -ExecutionPolicy Bypass -c ...`를 실행해 profile/PSModulePath 오염과 `Get-FileHash` 부재를 그대로 맞을 수 있으므로, sync-ai-clis가 같은 공식 installer URL을 직접 `-NoProfile` + `Get-FileHash` fallback 래퍼로 실행한다.
...
- [x] **Codex 네이티브 업데이트**: 해소(2026-06-12, TD-003; 2026-07-02 보완) — mac/linux native는 `codex update` 공식 self-update 서브커맨드를 사용한다. Windows native는 Codex updater 내부 PowerShell 호출이 profile/PSModulePath 오염과 `Get-FileHash` 부재를 유발할 수 있어, sync-ai-clis가 같은 공식 `install.ps1`을 `CODEX_NON_INTERACTIVE=1` + `Command::powershell_installer` 래퍼로 직접 실행한다. standalone 설치기 재실행은 install/on_broken recovery 경로에도 유지된다.

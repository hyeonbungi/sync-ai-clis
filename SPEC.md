...
- **릴리스 단위**: `Cargo.toml` version과 `CHANGELOG.md`의 `## X.Y.Z - YYYY-MM-DD` 섹션을 함께 올린다 — dist가 이 섹션을 릴리스 노트로 자동 사용한다. 공개 레포에 `vX.Y.Z` 태그 푸시가 곧 릴리스 트리거다.
...
- [ ] **`doctor` 서브커맨드**: 깨진 설치에 더해 **중복 설치 진단** — 같은 도구가 brew·npm 등 여러 경로에 깔려 PATH 순서 탓에 구버전이 잡히는 문제를 탐지하고 정리 방향을 제안. `source.rs` 출처 분류 재사용 (추천 1순위)

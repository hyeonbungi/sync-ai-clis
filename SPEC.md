...
| Codex (OpenAI) | `codex` | ✅ | ✅ | ✅ | `chatgpt.com/codex/install.sh`/`.ps1` | (설치기 재실행 — TODO 확인) | npm `@openai/codex` · brew cask `codex`(mac만) |
| Gemini (Google) | `gemini` | ✅ | ✅ | ✅ | npm `@google/gemini-cli` | (없음 — npm/brew 재설치) | brew `gemini-cli`(mac/linux) · npm |
| Kiro (AWS) | `kiro-cli` | ✅ | ✅ | ✅ | `cli.kiro.dev/install` (Win은 **Win11**) | `kiro-cli update` (+백그라운드 자동) | **❌ 없음** |
...
> 명령은 2026-06 리서치 기준. "TODO 확인" 표시는 구현 시 실제 검증 필요.
...
- **update** 설치출처별: brew cask→`brew upgrade --cask codex` · npm→`npm i -g @openai/codex@latest` · native→설치기 재실행 (**TODO**: `codex` 자체 업데이트 서브커맨드 있는지 확인)
...
- 비고: 네이티브 self-update 없음. npm/brew 재설치로 갱신. 기본 채널은 npm(보편), brew로 깔렸으면 brew

### 7.4 Kiro (`kiro-cli`, AWS — 구 Amazon Q CLI)
- **install** mac/linux: `curl -fsSL https://cli.kiro.dev/install | bash` · win: **Win11** PowerShell/Windows Terminal(동일 설치기 또는 플랫폼 빌드 — **TODO** Win 정확 명령 확인) · linux: glibc<2.34면 musl 변형
- **update**: `kiro-cli update` (+ 백그라운드 자동, 종료 시 적용). `self_updates = true`
- **Homebrew/npm 없음**
...
- [ ] **Codex 네이티브 업데이트**: 네이티브 설치 시 업데이트가 "설치기 재실행"인지, `codex` 자체 서브커맨드가 있는지 확인
- [ ] **Kiro Windows 정확 명령**: Win11 설치/업데이트 정확한 PowerShell 명령 확인 (`cli.kiro.dev/install`이 win도 커버하는지)
...
- [ ] **도구별 `install_dir` 확정**: PATH 미반영 대비 절대경로 재확인용 — `agy`=`~/.local/bin`(확인됨) 외 claude·codex·kiro·gemini 실제 설치 위치 확인
- [ ] **출처 분류: brew formula의 동봉 node_modules 오분류** (2026-06-11 실사용 발견): `gemini-cli`처럼 npm 패키지를 libexec에 동봉하는 brew formula는 심링크를 끝까지 풀면 `…/Cellar/<formula>/…/node_modules/…`가 되어 npm 마커가 brew 마커를 이긴다 → 업데이트가 `brew upgrade` 대신 `npm i -g`로 가고(중복 설치 유발 — 아래 prefix 항목과 결합 시 실제 발생), doctor의 출처 라벨도 틀린다. 수정 방향: `/cellar/`·`/caskroom/`을 `node_modules`보다 **먼저** 검사(formula/cask 경로는 결정적), 일반 homebrew prefix 검사는 지금처럼 node_modules 뒤에 둬서 brew-node 경유 npm 글로벌(`…/lib/node_modules/…`, Cellar 없음)은 계속 npm으로
- [ ] **npm 채널의 prefix 고정** (2026-06-11 실사용 발견): `npm install -g`는 "활성 npm"의 prefix에 설치하므로, nvm 등 다중 node 환경에서는 기존 설치(예: brew-node prefix)를 갱신하는 대신 **다른 prefix에 새 사본**을 만든다 — sync가 스스로 중복 설치를 만들 수 있음(doctor가 첫 실전에서 검출). 수정 방향: npm 채널 업데이트 시 기존 바이너리 경로에서 소유 npm을 역추적해(`node_modules` 상위의 `bin/npm`) 그 npm 절대경로로 실행
...
- [ ] **`--check` 모드**: 아무것도 바꾸지 않고 업데이트 가능 여부를 종료코드로만 반환 — 쉘 프롬프트 배지·CI 게이트·cron 알림용 부품. 데몬 비목표를 유지한 채 "백그라운드 상태 인지"를 사용자가 직접 조립하게 함 (추천 2순위)
...
- [ ] **설치 스크립트 변경 감지**: 원격 설치 스크립트를 실행 전 캐시해두고 다음 실행 때 내용이 달라졌으면 경고+diff 표시 — `curl | bash` 불안에 대한 실질 방어선, §5.5의 네 번째 기둥 후보 (차별화 최대, 설계 논의 필요)
...
- Codex CLI: https://developers.openai.com/codex/cli · https://www.npmjs.com/package/@openai/codex · https://github.com/openai/codex
...
- Kiro CLI: https://kiro.dev/docs/cli/installation/ · https://kiro.dev/blog/cli-2-0/ (Windows 지원)
